...
def build_spectral_norm_table(gamma, edges, nbins):
    # norm depends only on (gamma, bin edges), which are fixed for the whole
    # job: precompute it once per (binning, spectral index) instead of
    # recomputing log/power terms on every single event/fill.
    norms = array('d', [0.] * nbins)
    for ib in range(nbins):
        e1 = edges[ib]
        e2 = edges[ib + 1]
        norms[ib] = (gamma - 1.) * math.log(e2 / e1) / (e1 ** (1. - gamma) - e2 ** (1. - gamma))
    return norms


def spectral_weight_fast(E, gamma, edges, norm_table):
    # weight for a power-law spectrum normalized in [Emin, Emax] from edges,
    # using a precomputed per-bin normalization table (see build_spectral_norm_table)
    bin_idx = np.searchsorted(edges, E, side='right') - 1
    if bin_idx < 0:
        bin_idx = 0
    if bin_idx >= len(norm_table):
        bin_idx = len(norm_table) - 1
    return norm_table[bin_idx] * E ** (1. - gamma)
...
def fill_energy_cut(idx, etot, etot_truth, good_event, is_mc,
    ...
    for bname in energy_binnings:
        h_energy_truth[idx][bname].Fill(etot_truth)
        weights_for_bname = event_weights[bname]
        for tag, weight in weights_for_bname.items():
...
def compute_bgo_energies(pev, bgorec, bgo_api, is_mc, has_quenching, mc_particle_Z):
    raw_total_mev = bgorec.GetTotalEnergy()
    etot = raw_total_mev / 1000.

    etot_satcorr_ions = etot
    etot_satcorr_ions2 = etot
    if raw_total_mev > 1e6:
        ...
        bgo_api.Predict(mc_particle_Z, method="ions2")
        if bgo_api.IsSaturated():
            etot_satcorr_ions2 = bgo_api.GetReconstructedBGOE() / 1000.

    # Quenching branch: only for MC and only if available
    etot_quench = etot
    etot_quench_satcorr_ions = etot
    etot_quench_satcorr_ions2 = etot
    if is_mc and has_quenching:
        try:
            ...
            etot_quench_satcorr_ions2 = etot_quench
            if quench_rec.GetTotalEnergy() > 1e6:
                ...
                bgo_api.Predict(mc_particle_Z, method="ions2") # Z is a dummy value here. New ML SATcorr (0.5.0) N.B. without quenching!
                if bgo_api.IsSaturated():
                    etot_quench_satcorr_ions2 = bgo_api.GetReconstructedBGOE() / 1000.
        
        except Exception:
            etot_quench = etot
            etot_quench_satcorr_ions = etot_satcorr_ions
            etot_quench_satcorr_ions2 = etot_satcorr_ions2
...
def main(args=None):
    ...
    # Tabelle di normalizzazione per i pesi spettrali: norm dipende solo da
    # (gamma, bordi del bin), quindi va calcolata una sola volta per tutto il
    # job, non ad ogni evento/fill (vedi spectral_weight_fast).
    spectral_norm_tables = {}
    for bname, (nbins, edges) in energy_binnings.items():
        spectral_norm_tables[bname] = {}
        for tag, gamma in spectral_indices.items():
            spectral_norm_tables[bname][tag] = build_spectral_norm_table(gamma, edges, nbins)
    ...
    fBGO_EnergyG_SatCorr_ML_ions2 = make_branch(newtree, 'BGO_EnergyG_SatCorr_ML_ions2', 'd', None, -999.)
    
    fBGO_EnergyG_Quench = None
    fBGO_EnergyG_QuenchSatCorr_ML_ions = None
    fBGO_EnergyG_QuenchSatCorr_ML_ions2 = None
    if is_mc:
        ...
        fBGO_EnergyG_QuenchSatCorr_ML_ions2 = make_branch(newtree, 'BGO_EnergyG_QuenchSatCorr_ML_ions2', 'd', None, -999.)
    ...
    fBGO_cbgostk_ML = make_branch(newtree, 'BGO_cbgostk_ML', 'd', 14, -999.)
    ...
    fSTK_hitClusterSum = make_branch(newtree, 'STK_hitClusterSum', 'i', 2, -999)
    fSTK_hasXYclusters = make_branch(newtree, 'STK_hasXYclusters', 'i', None, 0)
    ...
    for iev in xrange(0, nevents):
        ...
        fBGO_EnergyG_SatCorr_ML_ions2[0] = -999.
        if is_mc:
            fBGO_EnergyG_Quench[0] = -999.
            fBGO_EnergyG_QuenchSatCorr_ML_ions[0] = -999.
            fBGO_EnergyG_QuenchSatCorr_ML_ions2[0] = -999.
        ...
        (etot,
         ...
         etot_quench,
         etot_quench_satcorr_ions,
         etot_quench_satcorr_ions2) = compute_bgo_energies(
            pev, bgorec, bgo_api, is_mc, has_quenching, mc_particle_Z
        )
        ...
        if is_mc:
            ...
            for bname in energy_binnings:
                event_weights[bname] = {}
                norm_tables_for_bname = spectral_norm_tables[bname]
                for tag, gamma in spectral_indices.items():
                    event_weights[bname][tag] = spectral_weight_fast(
                        etot_truth, gamma, energy_binnings[bname][1], norm_tables_for_bname[tag]
                    )
        ...
        x_projection_fromBGO_to_BGO_ana =  bgorec.GetInterceptXZ() + BGOzBot* bgorec.GetSlopeXZ()
        y_projection_fromBGO_to_BGO_ana =  bgorec.GetInterceptYZ() + BGOzBot* bgorec.GetSlopeYZ()
        x_projection_fromBGO_to_BGOTop_ana =  bgorec.GetInterceptXZ() +BGOzTop  * bgorec.GetSlopeXZ()
        y_projection_fromBGO_to_BGOTop_ana =  bgorec.GetInterceptYZ() +BGOzTop  * bgorec.GetSlopeYZ()
        ...
        if ((pevspace_api.GetDirectionBGOSlopeX() == 0 and pevspace_api.GetDirectionBGOInteceptX() == 0) or
            ...
            continue
        # anche analitico:
        if ((bgorec.GetSlopeXZ()==0 and bgorec.GetInterceptXZ()==0) or 
                (bgorec.GetSlopeYZ()==0 and bgorec.GetInterceptYZ()==0)): 
            continue
        ...
        v_bgolayer = np.array([bgorec.GetELayer(ibgo) for ibgo in range(14)])
        ...
        for ilay in xrange(1, 4):
            v_bgolayer_bars = np.array([bgorec.GetEdepPos(ilay, ibar) for ibar in xrange(0, 23)])
        ...
        MaxEBarLayer = [0, 0, 0]
        bgohits = pev.pEvtBgoHits()
        n_bgohits = pev.NEvtBgoHits()
        for ihit in xrange(0, n_bgohits):
            hitE = bgohits.fEnergy[ihit]
            layID = bgohits.GetLayerID(ihit)
            if layID == 1 or layID == 2 or layID == 3:
                if hitE > MaxEBarLayer[layID - 1]:
                    barNumberMaxEBarLayer[layID - 1] = (bgohits.fGlobalBarID[ihit] >> 6) & 0x1f
        ...
        fBGO_slopeX_ML[0] = pevspace_api.GetDirectionBGOSlopeX()
        fBGO_slopeY_ML[0] = pevspace_api.GetDirectionBGOSlopeY()
        fBGO_slopeXZ_analy[0] = bgorec.GetSlopeXZ()
        fBGO_slopeYZ_analy[0] = bgorec.GetSlopeYZ()
        fBGO_theta_analy[0] = np.arctan(np.sqrt(bgorec.GetSlopeXZ() ** 2 + bgorec.GetSlopeYZ() ** 2)) * 180 / np.pi
        fBGO_phi_analy[0] = math.atan2(bgorec.GetSlopeYZ(), bgorec.GetSlopeXZ()) * 180 / np.pi
        fBGO_interceptXZ_analy[0] = bgorec.GetInterceptXZ()
        fBGO_interceptYZ_analy[0] = bgorec.GetInterceptYZ()
        ...
        #    continue
        fSTK_hitClusterSum[0] = hitClusterSum[0] #X
        fSTK_hitClusterSum[1] = hitClusterSum[1] #Y
        fSTK_hasXYclusters[0] = int(hitClusterSum[0] > 0 and hitClusterSum[1] > 0)
        ...
        psd_vec_proj0X = [[] for _ in range(2)]
        ...
        psd_vec_proj0Y = [[] for _ in range(2)]
        psd_vec_positionY = [[] for _ in range(2)]
        psd_bar_id_y = [[] for _ in range(2)]

        psdhits = pev.pEvtPsdHits()
        n_psdhits = pev.NEvtPsdHits()
        for ipsd in xrange(0, n_psdhits):
            if psdhits.IsHitMeasuringX(ipsd):
                lengthX = [-99999., -99999.]
                array_lengthX = array('d', lengthX)
                hitZ = psdhits.GetHitZ(ipsd)
                if hitZ < -298.49 and hitZ > -298.51:
                    npsdX = 0
                if hitZ < -284.49 and hitZ > -284.51:
                    npsdX = 1

                if is_mc:
                    crossingX = DmpVSvc.gPsdECor.GetPathLPMC(psdhits.fGlobalBarID[ipsd], track_slope, track_impact_point, array_lengthX)
                else:
                    crossingX = DmpVSvc.gPsdECor.GetPathLengthPosition(psdhits.fGlobalBarID[ipsd], track_slope, track_impact_point, array_lengthX)

                if crossingX == True:
                    psd_vec_chargeX[npsdX].append(psdhits.fEnergy[ipsd])
                    psd_vec_chargeX0[npsdX].append(psdhits.fES0[ipsd])
                    psd_vec_chargeX1[npsdX].append(psdhits.fES1[ipsd])
                    psd_vec_gidX[npsdX].append(psdhits.fGlobalBarID[ipsd])
                    psd_bar_id_x[npsdX].append(psdhits.GetHittedBarNumber())
                    psd_vec_pathlengthX[npsdX].append(array_lengthX[1])
                    psd_vec_proj0X[npsdX].append(array_lengthX[0])
                    psd_vec_positionX[npsdX].append(psdhits.GetHitX(ipsd))

            elif psdhits.IsHitMeasuringY(ipsd):
                lengthY = [-99999., -99999.]
                array_lengthY = array('d', lengthY)
                hitZ = psdhits.GetHitZ(ipsd)
                if hitZ < -324.69 and hitZ > -324.71:
                    npsdY = 0
                if hitZ < -310.69 and hitZ > -310.71:
                    npsdY = 1

                if is_mc:
                    crossingY = DmpVSvc.gPsdECor.GetPathLPMC(psdhits.fGlobalBarID[ipsd], track_slope, track_impact_point, array_lengthY)
                else:
                    crossingY = DmpVSvc.gPsdECor.GetPathLengthPosition(psdhits.fGlobalBarID[ipsd], track_slope, track_impact_point, array_lengthY)

                if crossingY == True:
                    psd_vec_chargeY[npsdY].append(psdhits.fEnergy[ipsd])
                    psd_vec_chargeY0[npsdY].append(psdhits.fES0[ipsd])
                    psd_vec_chargeY1[npsdY].append(psdhits.fES1[ipsd])
                    psd_vec_gidY[npsdY].append(psdhits.fGlobalBarID[ipsd])
                    psd_bar_id_y[npsdY].append(psdhits.GetHittedBarNumber())
                    psd_vec_pathlengthY[npsdY].append(array_lengthY[1])
                    psd_vec_proj0Y[npsdY].append(array_lengthY[0])
                    psd_vec_positionY[npsdY].append(psdhits.GetHitY(ipsd))
        ...
        for ipsd in xrange(0, 2):
            if len(psd_vec_chargeY[ipsd]) > 0:
                ...
                proj0Y = psd_vec_proj0Y[ipsd][pos_max_len]
                pathY = psd_vec_pathlengthY[ipsd][pos_max_len]

                # crossingY was already True when this entry was stored in the
                # first PSD-hit loop, so the path length / projection does not
                # need to be recomputed here (point 4).
                PsdEC_tmpY = DmpVSvc.gPsdECor.GetPsdECor(psd_vec_gidY[ipsd][pos_max_len], proj0Y / 10.)

                AttCorY0 = DmpVSvc.gPsdECor.GetPsdMipAttESp3(psd_vec_gidY[ipsd][pos_max_len], 0, proj0Y)
                AttCorY1 = DmpVSvc.gPsdECor.GetPsdMipAttESp3(psd_vec_gidY[ipsd][pos_max_len], 1, proj0Y)
                ...
                if len(psd_vec_chargeY0[ipsd]) > 0:
                    psdchargeY0_corr[ipsd] = np.sqrt(psd_vec_chargeY0[ipsd][pos_max_len] / AttCorY0 * 10. / pathY)
                    if is_data:
                ...
                if len(psd_vec_chargeY1[ipsd]) > 0:
                    psdchargeY1_corr[ipsd] = np.sqrt(psd_vec_chargeY1[ipsd][pos_max_len] / AttCorY1 * 10. / pathY)
                    if is_data:
                ...
                if is_data:
                    psdchargeY01_corr[ipsd] = DmpVSvc.gPsdECor.GetPosCorrectedCharge(lay, bar, proj0Y, psdchargeY01_corr[ipsd])
                psdchargeY_proj[ipsd] = proj0Y
                psdY_pathlength[ipsd] = pathY
                ...
                if psd_vec_gidY[ipsd][pos_max_len] == 512 and proj0Y != -99999.:
                    h_psd_corr_ID512.Fill(proj0Y, PsdEC_tmpY)

            if len(psd_vec_chargeX[ipsd]) > 0:
                pos_max_len = np.argmax(psd_vec_pathlengthX[ipsd])
                proj0X = psd_vec_proj0X[ipsd][pos_max_len]
                pathX = psd_vec_pathlengthX[ipsd][pos_max_len]

                # crossingX was already True when this entry was stored in the
                # first PSD-hit loop, so the path length / projection does not
                # need to be recomputed here (point 4).
                PsdEC_tmpX = DmpVSvc.gPsdECor.GetPsdECor(psd_vec_gidX[ipsd][pos_max_len], proj0X / 10.)

                AttCorX0 = DmpVSvc.gPsdECor.GetPsdMipAttESp3(psd_vec_gidX[ipsd][pos_max_len], 0, proj0X)
                AttCorX1 = DmpVSvc.gPsdECor.GetPsdMipAttESp3(psd_vec_gidX[ipsd][pos_max_len], 1, proj0X)
                ...
                if len(psd_vec_chargeX0[ipsd]) > 0:
                    psdchargeX0_corr[ipsd] = np.sqrt(psd_vec_chargeX0[ipsd][pos_max_len] / AttCorX0 * 10. / pathX)
                    if is_data:
                ...
                if len(psd_vec_chargeX1[ipsd]) > 0:
                    psdchargeX1_corr[ipsd] = np.sqrt(psd_vec_chargeX1[ipsd][pos_max_len] / AttCorX1 * 10. / pathX)
                    if is_data:
                ...
                if is_data:
                    psdchargeX01_corr[ipsd] = DmpVSvc.gPsdECor.GetPosCorrectedCharge(lay, bar, proj0X, psdchargeX01_corr[ipsd])
                psdchargeX_proj[ipsd] = proj0X
                psdX_pathlength[ipsd] = pathX
                ...
                if psd_vec_gidX[ipsd][pos_max_len] == 5504 and proj0X != -99999.:
                    h_psd_corr_ID5504.Fill(proj0X, PsdEC_tmpX)
        ...
        vec_psdY_max = []
        for ipsd in xrange(0, n_psdhits):
            if psdhits.GetLayerID(ipsd) == 0:
                vec_psdY_max.append(psdhits.fEnergy[ipsd])
            elif psdhits.GetLayerID(ipsd) == 1:
                vec_psdX_max.append(psdhits.fEnergy[ipsd])
        ...
        for ipsd in xrange(0, n_psdhits):
            hitZ = psdhits.GetHitZ(ipsd)
            hitE = psdhits.fEnergy[ipsd]
            if hitZ == -324.7:
                if fPSD_EmaxY[0] < hitE:
                    fPSD_EmaxY[0] = hitE
                    fPSD_cpsdmax[0] = psdhits.GetHitY(ipsd)
            if hitZ == -310.7:
                if fPSD_EmaxY[1] < hitE:
                    fPSD_EmaxY[1] = hitE
                    fPSD_cpsdmax[1] = psdhits.GetHitY(ipsd)
            if hitZ == -298.5:
                if fPSD_EmaxX[0] < hitE:
                    fPSD_EmaxX[0] = hitE
                    fPSD_cpsdmax[2] = psdhits.GetHitX(ipsd) + 0.4
            if hitZ == -284.5:
                if fPSD_EmaxX[1] < hitE:
                    fPSD_EmaxX[1] = hitE
                    fPSD_cpsdmax[3] = psdhits.GetHitX(ipsd) + 0.6
        ...
        layer_hits = bgorec.GetLayerHits()
        for ibgo in xrange(0, 14):
            fBGO_EneLay[ibgo] = bgorec.GetELayer(ibgo) / 1000.
            fBGO_nhits_layers[ibgo] = layer_hits[ibgo]
            for ibar in xrange(0,22):
                fBGO_EneBars[(ibgo*22)+ibar] = bgorec.GetEdep(ibgo,ibar)/1000.
        ...
        for ihit in xrange(0, n_bgohits):
            layID = bgohits.GetLayerID(ihit)
            hitE = bgohits.fEnergy[ihit]
            barNumber = (bgohits.fGlobalBarID[ihit] >> 6) & 0x1f
            EBarLayers[layID].append(hitE)
            PosBarLayers[layID].append(barNumber)
            PosXBarLayers[layID].append(bgohits.GetHitX(ihit))
            PosYBarLayers[layID].append(bgohits.GetHitY(ihit))
        ...
        for ibgo in xrange(0, 14):
            ...
            if ibgo % 2 == 0:
                ...
                fBGO_cbgostk_ML[ibgo] = ( pevspace_api.GetDirectionBGOInteceptY() + pevspace_api.GetDirectionBGOSlopeY() * zbgo[ibgo] )
            if ibgo % 2 == 1:
                fBGO_cbgostk[ibgo] = track_impact_point.x() + track_slope.x() * (zbgo[ibgo] - track_impact_point.z())
                fBGO_cbgostk_ML[ibgo] = ( pevspace_api.GetDirectionBGOInteceptX() + pevspace_api.GetDirectionBGOSlopeX() * zbgo[ibgo] )
        ...
        Xtr = sumRMS * sumRMS * sumRMS * sumRMS * (sum(EBarLayers[13]) / bgorec.GetTotalEnergy()) / 8000000.
        ...
        fBGO_EnergyG_SatCorr_ML_ions2[0] = etot_satcorr_ions2

        if is_mc:
            fBGO_EnergyG_Quench[0] = etot_quench
            fBGO_EnergyG_QuenchSatCorr_ML_ions[0] = etot_quench_satcorr_ions
            fBGO_EnergyG_QuenchSatCorr_ML_ions2[0] = etot_quench_satcorr_ions2
        ...
        for ibgo in range(14):
            h_bgo_ene_lay[ibgo].Fill((bgorec.GetELayer(ibgo)) / 1000.)
