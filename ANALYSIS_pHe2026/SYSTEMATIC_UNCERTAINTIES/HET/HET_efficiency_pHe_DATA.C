...
Int_t    BGO_HET, BGO_UBT, STK_ntrack;
...
skim->SetBranchStatus("BGO_UBT",                      1);
...
skim->SetBranchAddress("BGO_UBT",                      &BGO_UBT);
...
for (Long64_t i = 0; i < nEntries; i++) {
    ...
    if (passUBT)
        h1_Unb->Fill(BGO_E_corr);

    if (passHET)
        h1Nobs->Fill(BGO_E_corr);

    if (passUBT && passHET)
        h1Nobs_And->Fill(BGO_E_corr);
