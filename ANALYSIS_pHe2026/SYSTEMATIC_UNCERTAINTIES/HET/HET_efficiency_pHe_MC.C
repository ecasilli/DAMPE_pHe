...
void ProcessMCSpecies(TChain **chains,
    ...
    for (Int_t iset = 0; iset < nsets; ++iset) {
        ...
        chain->SetBranchStatus("BGO_UBT",                      1);
        ...
        chain->SetBranchAddress("BGO_UBT",                      &BGO_UBT);
        ...
        for (Long64_t ievt = 0; ievt < nEntries; ++ievt) {
            ...
            Double_t weight = sampleNorm[iset] * TMath::Power(MC_EnergyT, -1.7);
            ...
            // Tutti i tagli offline sono stati superati.
            const Bool_t passUBT = (BGO_UBT > 0);
            const Bool_t passHET = (BGO_HET > 0);

            if (passUBT)
                h1_Unb->Fill(BGO_E_corr, weight);

            if (passHET)
                h1Nobs->Fill(BGO_E_corr, weight);

            if (passUBT && passHET)
                h1Nobs_And->Fill(BGO_E_corr, weight);
...
void HET_efficiency_pHe_MC(){
    ...
    TH1D *h1_Unb = new TH1D("h1_Unb", "Unbias-Trig (p+He); BGO deposited energy [GeV]; Events",noe, Ebin);
    h1_Unb->Sumw2();

    TH1D *h1Nobs = new TH1D("h1Nobs", "HE-Trig (p+He); BGO deposited energy [GeV]; Events",noe, Ebin);
    h1Nobs->Sumw2();

    TH1D *h1Nobs_And = new TH1D("h1Nobs_And", "HE*Unbias-Trig (p+He); BGO deposited energy [GeV]; Events",noe, Ebin);
    h1Nobs_And->Sumw2();

    TH1D *h1_HET_Eff = new TH1D("h1_HET_Eff", "HET-Eff vs Ebgo (p+He)", noe, Ebin);
    h1_HET_Eff->Sumw2();
