...
//			E_BGOEnergy[i] = (EnergyF[i] - EnergyI[i])/2.;
	...
	//	TF1 *ChargeFitFuncH  = new TF1("ChargeFitFuncH","[0]+[1]*log10(x)+[2]*log10(x)**2+[3]*log10(x)**3+[4]*log10(x)**4",25.0,15000.0);
		...
		ChargeSelectionMPVH->SetTitle(" ;BGO Energy (GeV);MPV H (MeV)");
	...
	//	TCanvas *C2 = new TCanvas("C2","C2",1000,800);
		...
		*/
		...
		ChargeSelectionMPVHMC->SetTitle(" ;BGO Energy (GeV);MPV He (MeV)");
...
/*	
		...
		SigmaH->SetTitle(" ;BGO Energy (GeV); #sigma_{H} (MeV)");
	...
	//	SigmaH->Draw("AP");
		...
		SigmaHMC->SetTitle(" ;BGO Energy (GeV); #sigma_{HMC} (MeV)");
...
//		TF1 *ChargeFitFuncWidthH  = new TF1("ChargeFitFuncWidthH","[0]+[1]*log10(x)+[2]*log10(x)**2+[3]*log10(x)**3+[4]*log10(x)**4",25.0,39820.0);	
		...
		WidthH->SetTitle(" ;BGO Energy (GeV); Width H (MeV)");
...
*/
		...
		WidthHMC->SetTitle(" ;BGO Energy (GeV); Width He (MeV)");
...
//		FitFuncGausH->FixParameter(1,0.0);
		...
		GausSigmaH->SetTitle(" ;BGO Energy (GeV); #sigma_{gaus} H (MeV)");
...
*/		
		...
		GausSigmaHMC->SetTitle(" ;BGO Energy (GeV); #sigma_{gaus} He (MeV)");
